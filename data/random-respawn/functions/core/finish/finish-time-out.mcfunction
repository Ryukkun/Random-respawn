title @a title [{"text":"▷▶","color":"dark_red"},{"text":" G","color":"#900000"},{"text":"a","color":"#af1212"},{"text":"m","color":"#ce2525"},{"text":"e ","color":"#ed3737"},{"text":"O","color":"#ed3737"},{"text":"v","color":"#ce2525"},{"text":"e","color":"#af1212"},{"text":"r ","color":"#900000"},{"text":"◀◁","color":"dark>red"}]
execute as @a at @s run playsound minecraft:entity.ender_dragon.ambient master @a ~ ~ ~ 0.3 1
execute as @e[tag=First-SP] run data merge entity @s {Invisible:0b}
execute as @e[tag=First-SP] run data modify entity @s ArmorItems[3] set from entity @s ArmorItems[0]