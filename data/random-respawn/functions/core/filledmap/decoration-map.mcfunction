...
execute if score $gm F-Temp matches 1..4 run data modify entity @s ArmorItems[3] set from storage minecraft:random-respawn FilledMap[0]
...
### デコレーション実演
execute if score $gm F-Temp matches 1..4 run data modify entity @s ArmorItems[3].tag merge value {Decorations:[{x:0b,z:0b,type:4b,rot:180.0d,id:""}]}
...
# Give Map
execute if score $gm F-Temp matches 1..4 at @s run item replace entity @p armor.head from entity @s armor.head
