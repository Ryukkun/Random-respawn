...
# Filled Map デコレーション
execute if score $gm F-Temp matches 1..3 if data storage random-respawn {Setting:{MapEnable:1b}} if score $StopW20 F-Timer matches 0 at @a[team=Player,tag=Did-MCID,tag=!Did-Head] as @e[tag=ILoveFilledMap,sort=nearest,limit=1] at @s run function random-respawn:core/1-3/filledmap/decoration-map